...
Home has a compact header with a menu button at the top left. Its menu opens Settings, checks for updates, and offers Clear all data. Settings contains the dark mode switch and shows the installed app version, with room for more settings later. Clear all data requires confirmation, then resets all app-owned data and permissions on this phone, including jobs, drafts, photos, costs, and settings, as though the app had just been installed. Copies exported outside the app remain.
