...
Home has a compact header with a menu button at the top left. Its menu opens Settings, checks for updates, and offers Clear all data. The separate local development app is labeled **Job Tracker Local**, stays at version **0.0.0**, and omits the GitHub update action; it can be rebuilt and installed beside the published app without changing the published app or its data. Settings contains the dark mode switch and shows the installed app version, with room for more settings later. Clear all data requires confirmation, then resets all app-owned data and permissions on this phone, including jobs, drafts, photos, costs, and settings, as though the app had just been installed. Copies exported outside the app remain.
...
Use a multi-step flow with **Back**, **Next**, and visible progress. Going back preserves entered data. The user can leave and resume an unfinished draft; an unfinished draft does not appear as a planned job until setup is completed. On Home, swiping the unfinished draft card left offers deletion, which requires confirmation and removes photos stored only with that draft. Each step supports editing after creation from job details.
