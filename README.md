...
GitHub Actions builds, tests, and lints every push and pull request. Every successful push to `main` gets a new numbered GitHub Release with the installable APK. The workflow assigns an increasing app version and version code from its run number, creates the matching tag, and generates release notes from the changes. Other workflow runs can leave gaps in the release numbers. A local build uses the fallback version in `app/build.gradle.kts`; set `JOBTRACKER_VERSION_NAME` and `JOBTRACKER_VERSION_CODE` when building locally if it needs to update a newer installed release. Pull requests do not publish releases.
